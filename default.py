...
addon = xbmcaddon.Addon('plugin.video.EcuafullTotal')
...
SOURCES = [{"title": "EcuafullTotal", "url": "http://webtutoshugo.blogspot.com.es/p/ecuafulltotal101.html", "fanart": "http://2.bp.blogspot.com/-Ksh7bv7GEF8/Veo8yYkoC3I/AAAAAAAAEvY/cLN7evt_RYs/s1600/ecua.jpg","thumbnail": "http://2.bp.blogspot.com/-bnw-M5ZqOK4/Vh5KJb2lW1I/AAAAAAAAE2M/Gb-aXCkD4PA/s1600/icon.png"}]

# http://2.bp.blogspot.com/-Ksh7bv7GEF8/Veo8yYkoC3I/AAAAAAAAEvY/cLN7evt_RYs/s1600/ecua.jpg
# http://2.bp.blogspot.com/-bnw-M5ZqOK4/Vh5KJb2lW1I/AAAAAAAAE2M/Gb-aXCkD4PA/s1600/icon.png
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.EcuafullTotal-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(SLatina,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(EcuafullTotal,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        if os.path.exists(favorites) == True:
            ...
            if FAV == "[]":
                ...
                addDir('[COLOR red][B]- FAVORITES CHANNEL EcuafullTotal [/COLOR][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
                addDir('','',100,'',FANART,'','','','')
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('SLatina','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from SLatina Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('Add to SLatina Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(SLatina,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from SLatina Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to SLatina Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(SLatina,Failed to extract regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(SLatina,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
